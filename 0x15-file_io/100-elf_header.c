...
void print_entry(unsigned long int e_entry, unsigned char *e_ident)
{
	...
	if (e_ident[EI_DATA] == ELFDATA2MSB)
	{
		e_entry = ((e_entry * 256) & 0xFF00FF00) |
			  ((e_entry / 256) & 0xFF00FF);
		e_entry = (e_entry * 65536) | (e_entry / 65536);
	}
 ...
 *
 * Return: 0 on success If the file is not an ELF File or
...
int main(int __attribute__((__unused__)) ac, char **av)
{
	...
	readd = read(opened, h, sizeof(Elf64_Ehdr));
	if (readd == -1)
	{
		free(h);
		close_elf(opened);
